(function(lib){
   ...
   var parseEvents = lib.template.parsers[Node.ELEMENT_NODE].parseEvents;
   var parseCustomElement = lib.template.parsers[Node.ELEMENT_NODE].parseCustomElement
   var parseIf = lib.template.parsers[Node.ELEMENT_NODE].parseIf;
   var parseIterator = lib.template.parsers[Node.ELEMENT_NODE].parseIterator;

   function parse(elementNode, modelSpy, definitionInstance){
      if (parseCustomElement(elementNode, modelSpy, definitionInstance)){
         return;
      }
      if(parseIterator(elementNode, modelSpy)){
         return; //The actual element has been removed.
      }
      ...
      parseEvents(elementNode, definitionInstance);

      parseIf(elementNode, modelSpy);
   }
   ...
   /* When supported/adequate, use 'oninput' event to have direct feedback. */
   function changeEventName(elementNode){
      if('oninput' in elementNode &&
      /input/i.test(elementNode.tagName) &&
      !/radio/i.test(elementNode.type) &&
      !/checkbox/i.test(elementNode.type)){
         return 'input';
      }
      return 'change';
   }
