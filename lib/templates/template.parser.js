(function(lib){
   ...
   function analyse(template, modelSpy, definitionInstance){
      ...
      walkChilds(fragment, function(element){
         analyseElement(element, element.$$modelSpy || modelSpy, definitionInstance);
      });
   ...
   function walkChilds(element, callback){
      callback(element);
      var defineSpy = !!element.$$modelSpy;
      for (var i = 0 ; i < element.childNodes.length ; i++){
         var child = element.childNodes[i];
         if (defineSpy){
            child.$$modelSpy = child.$$modelSpy || element.$$modelSpy;
         }
         walkChilds(child, callback);
      }
   }
